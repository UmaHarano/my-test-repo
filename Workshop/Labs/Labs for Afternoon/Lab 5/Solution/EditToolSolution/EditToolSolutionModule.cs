...
{
    internal class EditToolSolutionModule : Module
    {
        ...
        private const string COMMENT_SETTINGS_PROJECT = "EditToolSolution_Settings";
        private const string MODULE_NAME = "EditToolSolution_Module";
        private static string GENCOMMENT_NAME = "GenerateComments";

        private static EditToolSolutionModule _this = null;
        ...
        private static async Task TrackEditEvents(EditEventArgs editEventArgs)
        {
            ...
            foreach (var mapMember in editEventArgs.Creates)
            {
                ...
                // the cast from MapMember to FeatureLayer is ok because we only entered layers of type FeatureLayer into the event subscription
                var featureLayer = mapMember.Key as FeatureLayer;
                if (featureLayer == null)
                {
                    // in case we have something different than a FeatureLayer, continue to the next layer
                    continue;
                }
                ...
                // get the feature class from the layer
                Table table = await featureLayer.GetTableAsync();
                int commentFieldIndex = ((TableDefinition)table.Definition).FindField(COMMENT_FIELD_NAME);
                ...
                editOperation.Callback(async context =>
                {
                    foreach (long featureOID in member.Value)
                    {
                        ...
                        // find the index for the comment field
                        // 
        ...
        /// </summary>
        public static EditToolSolutionModule Current
        {
            get
            {
                return _this ?? (_this = (EditToolSolutionModule)FrameworkApplication.FindModule(MODULE_NAME));
            }
        }
        ...
        public static async Task<Row> GetRowByIDAsync(this Table table, long oid)
        {
            ...
            await QueuingTaskFactory.StartNew(() =>
            {
                if (table.Definition is TableDefinition)
                {
                    ...
                    var queryFilter = new QueryFilter { WhereClause = tableDefinition.ObjectIDField + " = " + oid.ToString() };
